...
- 只删除完整、非空、使用已知模式且不严格扩权的请求。
- 真正扩权的请求保持不变，继续进入 DSH 原生审批流程。
- 孤立字段、空理由、未知模式、策略解析失败保持不变，由 DSH 核心验证或拒绝。
...
- 畸形请求仍然失败：这是预期行为；插件不会将无效安全请求降级为普通调用。
- 提权请求仍出现审批：当目标模式严格宽于当前模式时，插件会保留请求并交给 DSH 原生审批。
